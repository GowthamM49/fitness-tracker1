...
const Register = () => {
  ...
  /** Fields visible on each step — used so "Next" doesn't submit the whole form */
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateForm()) {
      // User may have jumped via stepper; show where to fix
      if (!formData.name.trim() || !formData.email || !formData.age || !formData.gender) {
        setActiveStep(0);
      } else if (!formData.password || !formData.confirmPassword) {
        setActiveStep(1);
      } else if (!formData.height || !formData.weight || !formData.fitnessGoal) {
        setActiveStep(2);
      }
      return;
    }
