...
router.get('/reports/user-progress', adminAuth, async (req, res) => {
  try {
    ...
    const progressData = await ProgressEntry.find(filter)
    ...
    const stats = {
      ...
      averageWeight: progressData.length > 0
        ? progressData.reduce((sum, p) => sum + (p.weight || 0), 0) / progressData.length
        : 0,
      totalWeightLoss: progressData.length > 1
        ? Math.max(0, progressData[progressData.length - 1].weight - progressData[0].weight)
        : 0
    };
